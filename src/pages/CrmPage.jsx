...
export default function CrmPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      // Ambil data Leads dan Customers secara bersamaan
      ...
      console.error("Gagal mengambil data CRM", err);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post(`${baseUrl}/api/crm/leads`, formData, { 
      ...
      alert("❌ Gagal menyimpan prospek: " + (err.response?.data?.error || err.message));
    } finally {
      setLoading(false);
    }
  };

  // Fungsi Ajaib: Mengubah Prospek menjadi Pelanggan Resmi
  const handleConvert = async (leadId, companyName) => {
    const confirmConvert = window.confirm(`Apakah Anda yakin klien "${companyName}" sudah DEAL / Tanda Tangan Kontrak?`);
    if (!confirmConvert) return;

    const token = localStorage.getItem('token');
    try {
      await axios.post(`${baseUrl}/api/crm/leads/${leadId}/convert`, {}, {
        headers: { Authorization: `Bearer ${token}` }
      });
      alert(`🎉 SELAMAT! "${companyName}" resmi menjadi Pelanggan Anda!`);
      fetchData(); // Refresh data (memindahkan dari tabel Lead ke Customer)
    } catch (err) {
      alert("❌ Gagal mengonversi klien: " + (err.response?.data?.error || err.message));
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button 
          ...
          className={`${showForm ? 'bg-red-500' : 'bg-indigo-600 hover:bg-indigo-700'} text-white px-4 py-2 rounded-lg font-bold shadow-sm transition`}
        >
      ...
      {showForm && (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 mb-6 border-t-4 border-t-indigo-500">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mb-8">
        <div className="bg-orange-50 p-4 border-b border-orange-100">
          <h2 className="font-bold text-orange-800">🎯 Daftar Prospek (Leads)</h2>
          <p className="text-xs text-orange-600">Tekan tombol "Jadikan Pelanggan" jika klien ini sudah deal.</p>
        </div>
        <table className="w-full text-left border-collapse">
          <thead className="bg-gray-50 text-gray-600 text-xs uppercase tracking-wider">
            <tr>
              <th className="p-4 font-semibold border-b">Perusahaan</th>
              <th className="p-4 font-semibold border-b">Kontak (PIC)</th>
              <th className="p-4 font-semibold border-b">Email / Phone</th>
              <th className="p-4 font-semibold border-b text-center">Status</th>
              <th className="p-4 font-semibold border-b text-right">Aksi Sales</th>
            </tr>
          </thead>
          <tbody className="text-sm divide-y divide-gray-100">
            {leads.length === 0 ? (
              <tr><td colSpan="5" className="p-6 text-center text-gray-400">Belum ada data prospek.</td></tr>
            ) : (
              leads.map((lead) => (
                <tr key={lead.ID || lead.id} className="hover:bg-gray-50">
                  ...
                  <td className="p-4">
                    <p className="text-blue-600">{lead.Email || lead.email}</p>
                    <p className="text-xs text-gray-500">{lead.Phone || lead.phone}</p>
                  </td>
                  ...
                  <td className="p-4 text-right">
                    {lead.Status !== 'converted' ? (
                      <button 
                        onClick={() => handleConvert(lead.ID || lead.id, lead.Company || lead.company)}
                        className="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded text-xs font-bold shadow-sm transition"
                      >
                        ✅ Jadikan Pelanggan
                      </button>
                    ) : (
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <table className="w-full text-left border-collapse">
          <thead className="bg-gray-50 text-gray-600 text-xs uppercase tracking-wider">
            <tr>
              <th className="p-4 font-semibold border-b">Nama Institusi / Pelanggan</th>
              <th className="p-4 font-semibold border-b">Kontak Personal</th>
              <th className="p-4 font-semibold border-b">Email</th>
              <th className="p-4 font-semibold border-b">No. Telepon</th>
            </tr>
          </thead>
          <tbody className="text-sm divide-y divide-gray-100">
            {customers.length === 0 ? (
              <tr><td colSpan="4" className="p-6 text-center text-gray-400">Belum ada pelanggan resmi.</td></tr>
            ) : (
              customers.map((cust) => (
