      ...
      {activeTab === 'boms' && (
        <div className="space-y-6">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-indigo-100">
            <h2 className="text-lg font-bold text-indigo-800 mb-4">Buat Resep Baru</h2>
            <form onSubmit={submitBom} className="space-y-4">
              ...
              <div className="bg-gray-50 p-4 border rounded">
                <h3 className="text-sm font-bold mb-2 flex justify-between">Komponen Bahan Baku<button type="button" onClick={handleAddComp} className="text-xs bg-indigo-500 text-white px-2 py-1 rounded">+ Tambah Bahan</button></h3>
              ...
              <button type="submit" disabled={loading} className="w-full bg-indigo-600 text-white font-bold py-2 rounded">Simpan Resep</button>
            </form>
          </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-gray-50 text-xs uppercase text-gray-600">
                  <tr><th className="p-4 border-b">Kode BOM</th><th className="p-4 border-b">Nama Resep</th><th className="p-4 border-b text-right">Aksi</th></tr>
                </thead>
                <tbody className="text-sm">
                  {boms.length === 0 ? <tr><td colSpan="3" className="p-6 text-center text-gray-400">Belum ada resep.</td></tr> : boms.map(b => (
                    <tr key={b.ID} className="hover:bg-gray-50 border-b">
                      <td className="p-4 font-bold text-indigo-600">{b.Code}</td>
                      <td className="p-4 font-medium">{b.Name}</td>
                      <td className="p-4 text-right">
                        <button onClick={() => deleteBom(b.ID, b.Name)} className="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-xs font-bold">🗑️ Hapus</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-emerald-100 border-t-4 border-t-emerald-500">
            ...
            <form onSubmit={submitOrder} className="grid grid-cols-4 gap-4">
              <div><label className="block text-xs font-bold mb-1">Pilih Resep</label><select required value={orderForm.bom_id} onChange={e => setOrderForm({...orderForm, bom_id: e.target.value})} className="w-full p-2 border rounded text-sm"><option value="">-- Pilih BOM --</option>{boms.map(b => <option key={b.ID} value={b.ID}>{b.Name}</option>)}</select></div>
              ...
              <tbody className="text-sm">
                {orders.map(o => (
                  <tr key={o.ID} className="hover:bg-gray-50 border-b">
                    ...
                    <td className="p-4 text-right">
                      {o.Status !== 'completed' ? (
                        <button onClick={() => completeProduction(o.ID, o.OrderNumber)} className="bg-blue-600 text-white px-3 py-1 rounded text-xs font-bold">🏭 Selesai</button>
