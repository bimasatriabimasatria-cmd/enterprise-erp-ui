...
export default function HrPage() {
  ...
  const fetchEmployees = async () => {
    ...
    try {
      const response = await axios.get(`${baseUrl}/api/hr/employees`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        nik: formData.nik, name: formData.name, position: formData.position,
        basic_salary: Number(formData.salary) || 0, hire_date: formData.hire_date
      };
      ...
      fetchEmployees();
    } catch (err) { alert("❌ Gagal menyimpan data: " + (err.response?.data?.error || err.message)); } 
  ...
  const handleEdit = (emp) => {
    ...
    setFormData({
      nik: emp.NIK || emp.nik || '', name: emp.Name || emp.name || '', position: emp.Position || emp.position || '',
  ...
  const handlePaySalary = (name, salary) => {
    if (window.confirm(`Cairkan gaji untuk ${name} sebesar Rp ${salary.toLocaleString('id-ID')}?`)) alert(`💸 BUKTI TRANSFER: Gaji berhasil dikirim ke rekening ${name}!`);
  };
  ...
  // 🚀 FUNGSI EXPORT DATA KARYAWAN
  const handleExportCSV = () => {
    const formattedData = employees.map((emp, index) => ({
      "No": index + 1, "NIK Karyawan": emp.NIK || emp.nik || "-", "Nama Lengkap": emp.Name || emp.name || "-",
      "Jabatan": emp.Position || emp.position || "-", "Gaji Pokok (Rp)": emp.BasicSalary || emp.basic_salary || 0,
      ...
      {selectedSlip && (
        <div className="hidden print:block p-10 font-sans text-black">
          <div className="flex justify-between items-end border-b-4 border-slate-800 pb-4 mb-8">
            <div><h1 className="text-4xl font-black tracking-widest text-slate-800">ENTERPRISE</h1><p className="text-sm font-bold text-slate-500 tracking-widest">ERP SYSTEM V2.0 INDONESIA</p></div>
            <div className="text-right"><h2 className="text-2xl font-bold text-slate-800">SLIP GAJI KARYAWAN</h2><p className="text-sm text-slate-600">Periode: {new Date().toLocaleDateString('id-ID', { month: 'long', year: 'numeric' })}</p></div>
          </div>
          <div className="grid grid-cols-2 gap-4 mb-8 bg-slate-50 p-4 border rounded-lg">
            <div><p className="text-xs font-bold text-slate-500 uppercase">Nama Karyawan</p><p className="text-lg font-bold text-slate-800">{selectedSlip.Name || selectedSlip.name}</p></div>
            <div><p className="text-xs font-bold text-slate-500 uppercase">Nomor Induk (NIK)</p><p className="text-lg font-mono text-slate-800">{selectedSlip.NIK || selectedSlip.nik}</p></div>
            <div><p className="text-xs font-bold text-slate-500 uppercase">Jabatan</p><p className="text-lg text-slate-800 uppercase">{selectedSlip.Position || selectedSlip.position}</p></div>
            <div><p className="text-xs font-bold text-slate-500 uppercase">Tanggal Bergabung</p><p className="text-lg text-slate-800">{new Date(selectedSlip.HireDate).toLocaleDateString('id-ID')}</p></div>
          </div>
          <table className="w-full text-left border-collapse mb-8">
            <thead><tr className="bg-slate-200 border-b-2 border-slate-800"><th className="p-3 font-bold text-slate-800 uppercase text-sm">Deskripsi Pendapatan</th><th className="p-3 font-bold text-slate-800 uppercase text-sm text-right">Jumlah (Rp)</th></tr></thead>
            <tbody>
              <tr className="border-b"><td className="p-3 text-slate-700">Gaji Pokok (Basic Salary)</td><td className="p-3 font-mono text-right text-slate-800">Rp {(selectedSlip.BasicSalary || selectedSlip.basic_salary || 0).toLocaleString('id-ID')}</td></tr>
              <tr className="border-b"><td className="p-3 text-slate-700">Tunjangan Operasional (Simulasi)</td><td className="p-3 font-mono text-right text-slate-800">Rp 0</td></tr>
              <tr className="border-b"><td className="p-3 text-slate-700">Potongan / Pajak (Simulasi)</td><td className="p-3 font-mono text-right text-red-600">- Rp 0</td></tr>
              <tr className="bg-slate-100 border-t-2 border-slate-800"><td className="p-4 font-black text-slate-800 text-lg">PENERIMAAN BERSIH</td><td className="p-4 font-black text-slate-800 text-right text-xl">Rp {(selectedSlip.BasicSalary || selectedSlip.basic_salary || 0).toLocaleString('id-ID')}</td></tr>
            </tbody>
          </table>
          <div className="flex justify-between mt-16 px-10">
            <div className="text-center"><p className="mb-20 text-sm font-bold text-slate-600">Penerima,</p><p className="font-bold border-b border-slate-800 px-4 inline-block">{selectedSlip.Name || selectedSlip.name}</p></div>
            <div className="text-center"><p className="mb-20 text-sm font-bold text-slate-600">Manager HRD & Finance,</p><p className="font-bold border-b border-slate-800 px-4 inline-block">Enterprise ERP Admin</p></div>
          </div>
        </div>
      )}
      ...
      <div className="space-y-6 print:hidden">
        <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 border-b pb-4">
          ...
          <div className="flex space-x-2">
            <button onClick={() => setActiveTab('directory')} className={`px-3 md:px-4 py-2 font-bold rounded-lg transition text-sm md:text-base ${activeTab === 'directory' ? 'bg-indigo-600 text-white shadow-md' : 'bg-gray-200 text-gray-700'}`}>📖 Direktori Karyawan</button>
            <button onClick={() => setActiveTab('payroll')} className={`px-3 md:px-4 py-2 font-bold rounded-lg transition text-sm md:text-base ${activeTab === 'payroll' ? 'bg-emerald-600 text-white shadow-md' : 'bg-gray-200 text-gray-700'}`}>💸 Proses Penggajian</button>
          </div>
        </div>
        ...
        {activeTab === 'directory' && (
          <div className="space-y-6 animate-fade-in-up">
            <div className="flex flex-wrap justify-end gap-3">
              ...
              <button onClick={() => {setShowForm(!showForm); setEditingId(null);}} className={`${showForm ? 'bg-red-500 hover:bg-red-600' : 'bg-indigo-600 hover:bg-indigo-700'} text-white px-3 md:px-4 py-2 rounded-lg font-bold shadow-sm transition text-sm md:text-base`}>
            ...
            {showForm && (
              <div className="bg-white p-6 rounded-xl shadow-sm border border-indigo-100 relative">
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="md:col-span-2"><label className="block text-xs font-semibold text-gray-600 mb-1">Gaji Pokok / Salary (Rp)</label><input type="number" min="0" required value={formData.salary} onChange={(e) => setFormData({...formData, salary: e.target.value})} className="w-full p-2 border rounded bg-gray-50" /></div>
                  <div className="md:col-span-2 mt-4"><button type="submit" disabled={loading} className="w-full bg-indigo-600 text-white font-bold py-3 rounded-lg hover:bg-indigo-700 transition">{loading ? 'Menyimpan...' : (editingId ? 'Simpan Perubahan' : 'Simpan Karyawan Baru')}</button></div>
                </form>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {employees.length === 0 ? (
                <div className="col-span-1 md:col-span-2 lg:col-span-3 bg-white p-8 rounded-xl shadow-sm border text-center text-gray-400 font-medium">Belum ada data karyawan.</div>
              ) : (
                employees.map((emp) => {
        ...
        {activeTab === 'payroll' && (
          <div className="bg-white rounded-xl shadow-sm border border-emerald-100 overflow-hidden border-t-4 border-t-emerald-500">
            <div className="p-4 md:p-5 bg-emerald-50 border-b border-emerald-100 flex flex-col md:flex-row justify-between md:items-center gap-2">
              <div><h2 className="font-bold text-emerald-800 text-lg">💸 Antrean Gaji Karyawan</h2></div>
              <div className="text-left md:text-right"><p className="text-xs text-emerald-700 font-bold uppercase">Total Tagihan Payroll</p><p className="text-xl font-black text-emerald-800">Rp {totalSalaryBurden.toLocaleString('id-ID')}</p></div>
            </div>
            
            {/* 🔥 BUNGKUSAN PELINDUNG TABEL MOBILE 🔥 */}
                    ...
                    return (
                      <tr key={emp.ID || emp.id} className="hover:bg-white transition-colors">
                        ...
                        <td className="p-4 text-center space-x-2 whitespace-nowrap">
                          <button onClick={() => handlePrintSlip(emp)} className="bg-slate-800 hover:bg-slate-900 text-white px-3 md:px-4 py-2 md:py-2.5 rounded-lg text-xs font-bold shadow-md transition-all">🖨️ Cetak Slip</button>
                          <button onClick={() => handlePaySalary(name, salary)} className="bg-emerald-500 hover:bg-emerald-600 text-white px-3 md:px-4 py-2 md:py-2.5 rounded-lg text-xs font-bold shadow-md transition-all">💸 Kirim Gaji</button>
                        </td>
                      </tr>
                    );
                  })}
