    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button 
          ...
          className={`${showForm ? 'bg-red-500' : 'bg-cyan-600 hover:bg-cyan-700'} text-white px-4 py-2 rounded-lg font-bold shadow-sm transition`}
        >
      ...
      <div className="grid grid-cols-2 gap-6 mb-6">
        <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 border-l-4 border-l-cyan-500">
          <p className="text-gray-500 text-sm font-semibold">Total Nilai Pembelian (Pengeluaran)</p>
          <p className="text-2xl font-bold text-gray-800 mt-1">Rp {totalSpent.toLocaleString('id-ID')}</p>
        </div>
        <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 border-l-4 border-l-amber-500">
          <p className="text-gray-500 text-sm font-semibold">Menunggu Pengiriman (Status Draft)</p>
          <p className="text-2xl font-bold text-gray-800 mt-1">{pendingPos} Dokumen PO</p>
        </div>
      </div>

      {showForm && (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 mb-6 border-t-4 border-t-cyan-500">
              ...
              pos.map((po) => (
                <tr key={po.ID} className="hover:bg-gray-50">
                  ...
                  <td className="p-4 text-center">
                    <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${po.Status === 'received' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
                  ...
                  <td className="p-4 text-right">
                    {po.Status === 'draft' ? (
                      <button 
                        ...
                        className="bg-emerald-500 hover:bg-emerald-600 text-white px-3 py-1 rounded text-xs font-bold shadow-sm transition"
                      >
                        📥 Terima Barang
                      </button>
                    ) : (
